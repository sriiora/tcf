...
class test_target(object):
    ...
    def _ip_addr_validate(self, _ip_addr):
        ...
        for ic_data in list(self.tags.get('interconnects', {}).values()):
            if not ic_data:
                continue
            for key, value in list(ic_data.items()):
    ...
    def _state_cleanup(self, force):
        ...
        for _, (thing_target, plugger) in list(self.things.items()):
            self._thing_unplug(thing_target, plugger)
        # if this target is a thing to other targets, unplug
        # itself from them
        for _, (target, plugger) in list(self.thing_to.items()):
