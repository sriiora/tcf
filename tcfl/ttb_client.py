    ...
    def send_request(self, method, url, data = None, files = None,
        ...
        if len(r.cookies) > 0:
            with self.lock:
                ...
                for cookie, value in list(r.cookies.items()):
...
def rest_shutdown(path):
    ...
    :type path: str
    """
    for rtb in list(rest_target_brokers.values()):
...
def _dict_print_dotted(d, _prefix = ""):
    for key, val in sorted(list(d.items()), key = lambda i: i[0]):
        prefix = _prefix + key
...
def rest_target_list_table(args, spec):
    ...
    l = []
    for rt_fullid, rt in sorted(iter(list(rest_target_broker.rts_cache.items())),
                                key = lambda x: x[0]):
        try:
...
def rest_broker_file_list(args):
    ...
    """
    rtb = _rest_target_broker_find_by_id_url(args.target)
    for name, hexdigest in list(rtb.rest_tb_file_list().items()):
