...
class target_c(object):
    ...
    def __init__(self, rt, testcase, bsp_model, target_want_name):
        ...
        for name, extension in list(self.__extensions.items()):
            try:
    ...
    def _kws_update_interconnect_addrs(self, interconnects):
        ...
        for ic_name in sorted(interconnects):
            for tg_ic_name, tg_ic_dict \
                in list(self.rt.get('interconnects', {}).items()):
                if tg_ic_name != ic_name:
                    continue
                for key, value in list(tg_ic_dict.items()):
...
def _target_app_add(obj, target_want_name, app_name, app_src):
    ...
    if app.driver_valid(app_name):
        ...
        if isinstance(app_src, dict):
            for bsp, src in list(app_src.items()):
                twapp[bsp] = (_app, src)
        else:
            twapp["*"] = (_app, app_src)
        # Consistency check
    ...
    **alphabetical order** (not in declaration order!). The methods
    ...
    def _configure_50_for_target(self, _target):
        ...
        for bsp, (_app, app_src, _) \
            in list(_target.bsps_stub.items()):
            if _app == None:
    ...
    def tags_set(self, _tags, origin = None, overwrite = True):
        ...
        for tagname, (value, _origin) in list(_tags.items()):
            assert isinstance(tagname, str), \
    ...
    def _selected_str(rt_selected):
        return " ".join([fullid + ":" + ",".join(bsp_models)
                         for fullid, bsp_models in list(rt_selected.items())])

    @staticmethod
    def _tg_str(icg):
        s=""
        for twn, (rt_full_id, bsp_model) in sorted(iter(list(icg.items())),
                                                   key = lambda x: x[0]):
            if bsp_model == None:
    ...
    def _target_wants_list_permutations(self, target_want_candidates, n,
        ...
        for twn in target_want_candidates:
            ...
            if mode == "one-per-type":
                ...
                if len(target_want_candidates) == 1:
                    _n = 0
                    for rt_fullid, rt in list(self.rt_all.items()):
        ...
        while itr < n:
            ...
            perm_types_id = " ".join([
                ...
                for twn, (rt_type, bsp_model) in sorted(iter(list(_types.items())),
                                                        key = lambda x: x[0])])
            ...
            if not self._phase_skip("configure"):
                if self.subtc:
                    self.report_info(
                        ...
                        " ".join(list(self.subtc.keys())), dlevel = 1)
                else:
                    self.report_info(
    ...
    def _clone(self):
        ...
        for subtc_name, subtc in list(self.subtc.items()):
    ...
    def _permutations_make(self, rt_all, rt_selected, ic_selected):
        ...
        if len(self._interconnects):
            ...
            ic_permutations = { "all": { } }
        for icgid, icg in list(ic_permutations.items()):
        ...
        for icgid, icg in list(ic_permutations.items()):
            for icwn, (rtic, _) in list(icg.items()):
        ...
        for icgid, rt_selected in list(icg_selected.items()):
            # Generate keywords that describe the current
            # interconnects
            kws_ics = dict()
            for icwn, ic in list(ic_permutations[icgid].items()):
    ...
    def _run_on_targets(self, tp, rt_all, rt_selected, ic_selected):
        ...
        try:
            ...
            for (icgid, tgid), tg in list(rt_permutations.items()):
                if tgid == None:
...
def _testcase_match_tags(tc, tags_spec, origin = None):
    ...
    if origin == None:
        origin = "[builtin]"
    kws = dict()
    for name, (value, _vorigin) in list(tc._tags.items()):
        kws[name] = value
...
def testcases_discover(tcs_filtered, args):
    ...
    for tc_path, tc in list(tcs.items()):
        try:
...
def _targets_discover(args, rt_all, rt_selected, ic_selected):
    ...
    if not args.target:
        ...
        for rt_fullid, bsp_models in list(ic_selected_all.items()):
            rt = rt_all[rt_fullid]
        ...
        # Select targets
        for rt_fullid, bsp_models in list(rt_selected_all.items()):
            rt = rt_all[rt_fullid]
            # Introduce two symbols after the ID and fullid, so "-t
            # TARGETNAME" works
            rt[rt_fullid] = True
