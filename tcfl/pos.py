...
def image_seed_match(lp, goal):
    ...
    scores = {}
    check_empties = {}
    for part_name, seed in list(lp.items()):
        score = 0
        ...
        if seedl[0] == goall[0]:
            ...
            scores[part_name] = 0
    if scores:
        selected, score = max(iter(list(scores.items())), key = operator.itemgetter(1))
...
def cmdline_pos_capability_list(args):
    if not args.target:
        for name, data in list(capability_fns.items()):
            for value, fn in list(data.items()):
                print("%s: %s @%s.%s()" % (
